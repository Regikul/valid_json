...
| Schema validated against its meta-schema | yes, at registration, and the failure carries the output of that check | no, malformed schemas surface as `schema_invalid` during validation | no |
