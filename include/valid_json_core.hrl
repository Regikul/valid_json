...
%% need_coverage говорит, ждёт ли покрытие этого поддерева `unevaluated*` выше
%% по обходу. Пока ждёт, handlers строят `evaluated`, а перебор ветвей нельзя
%% обрывать даже при формате flag: успех первой ветви `anyOf` не отменяет
%% аннотаций остальных. Без спроса `evaluated` остаётся neutral.
-record(eval_context, {
    schema            :: compiled(),
    node              :: addr(),
    keyword_location  :: [binary()],
