...
- [x] Реализовать доступ к таблице артефактов: чтение идёт функцией
  ...
  модуля-клиента нет: клиент — это вызывающий, а имя таблицы выводится из имени
  хранилища в одном месте, у управляющего.
- [x] Реализовать хранителя таблицы с `heir` и управляющий процесс —
  единственный writer. Хранитель принимает имя хранилища, создаёт под ним named
  `protected` set с `read_concurrency`, назначает `heir` на себя и отдаёт
  владение по тотальному `claim/1`. Управляющий забирает таблицу при старте,
  держит реестр и опции хранилища и остаётся единственным, кто в таблицу пишет.
- [x] Собрать `rest_for_one` supervision tree: хранитель перед управляющим. Есть
  супервизор одного хранилища с `child_spec/1` и `child_spec/2`, принимающим
  опции хранилища; стандартное хранилище `valid_json` поднимается в дереве
  библиотеки под `one_for_one` и опции берёт из app env либо из встроенных
  умолчаний.
- [ ] Реализовать первичную компиляцию реестра до объявления готовности. Пункт
  ждёт источников схем: хранилище стартует пустым и наполняется вызовами
  фасада, поэтому компилировать при старте пока нечего, а `handle_continue` и
  `wait(Timeout)` появятся вместе с источником.
...
- [ ] Реализовать фасад размещённого режима: `add`, `remove` и `validate_uri`
  для стандартного хранилища и те же операции с приставкой `store_` и именем
  хранилища первым аргументом.
...
- [~] Покрыть storage, ownership transfer, restart, reload, rollback и
  ...
  три опции хранилища и перезапуск управляющего с переживанием таблицы. Условие
  «без evaluator» не выдержано: пересобранный артефакт отличается от прежнего
  только содержимым IR, и проверки наблюдают его через `validate/3`.
