...
- [~] Реализовать первичную компиляцию реестра до объявления готовности.
  Сделана пересборка после перезапуска: `handle_continue` компилирует имена
  реестра, оставшиеся без артефакта, а отказ компиляции валит управляющего, а не
  поднимает хранилище с дырой. Остаток ждёт источников схем — стартовый набор
  документов хранилищу по-прежнему не передаётся, и `wait(Timeout)` появится
  вместе с ним.
