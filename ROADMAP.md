...
- [x] Проверить, достаточно ли пары schema/instance в cycle guard: достаточно.
  ...
  повтор действительно означает отсутствие прогресса. В кадре локация
  представлена глубиной: кадры guard'а лежат на одном пути обхода, а на одном
  пути глубина задаёт позицию однозначно.
...
- [x] Собрать `rest_for_one` supervision tree: хранители перед управляющим,
  первым — хранитель реестра. Есть супервизор одного хранилища с `child_spec/2`,
  принимающим опции хранилища, и фасад повторяет эту спеку как
  `store_child_spec/2`; спеки хранителей составляет сам супервизор, потому что
  перезапуск и остановка — его политика. Стандартное хранилище `valid_json`
  поднимается в дереве библиотеки под `one_for_one` и опции берёт из app env
  либо из встроенных умолчаний.
...
- [x] Замкнуть загрузчик каталога в объявленной области и научить его брать
  корень из `priv`: опция `{priv_dir, App, Path}` рядом с `{root, Path}`, путь
  внутри `priv` проверяется `filelib:safe_relative_path/2`, обе опции корня
  сразу или приложение вне кода — `badarg`. Обход различает записи по
  `file:read_link_info/1`, которая по ссылке не идёт, поэтому символьная ссылка
  не обходится и не читается, а становится отказом `{link, Path}`: молчаливый
  пропуск потерял бы документ, неотличимый по имени от схемы. Покрыто тестами
  загрузчика, форму `priv_dir` показывают README и `config/sys.config`, область
  и источник корня описаны в okf.
- [x] Развести физический слой хранения и виртуальный слой адресации: адресом
  схемы служит её каноническое имя — объявленный `$id`, а без него имя, которое
  дало хранилище, — и путь к файлу участвует в выборе имени последним доводом,
  когда других указаний нет. Отсюда `base_uri` становится обязательной опцией
  хранилища и передаётся только ею: им хранилище заявляет о принадлежности схем
  сервису, стандартное получает `https://valid_json.internal/schemas/`, а
  хранилище без него завершается badarg. Из контракта загрузчика уходит колбэк
  `base_uri/1` вместе с цепочкой из трёх источников и app env; загрузчик отдаёт
  только относительные имена. Завершающий слэш дописывается нормализацией,
  `base_uri` с query отвергается, поле документа `retrieval` называется
  `registered`, а реестр на один вызов компиляции заводится через
  `valid_json_store:temporary/0`.
...
- [x] Реализовать фасад размещённого режима: `add`, `remove` и `validate`
  ...
  `store_validate` сначала делает клиентский `lookup` и затем вызывает
  `valid_json_core:validate/3`. Добавлены проверки именованного и стандартного
  хранилищ.
- [x] Хранить встроенные метасхемы отдельно и публиковать их compiled form в
  защищённой ETS под собственной веткой супервизора: при старте приложения
  шестнадцать документов читаются и разбираются один раз, доверенно компилируются
  два корневых замыкания из восьми и семи resources, после чего одна immutable
  запись с bundle обоих dialect'ов ложится в таблицу. Владеет таблицей рабочий
  процесс ветки, а смерть его она переживает у хранителя по `heir`. `store()`
  читает промах из bundle мимо пользовательского реестра; встроенное имя нельзя
  переписать или удалить. Обращение к встроенной области при незапущенном
  приложении — ошибка вызывающего.
...
- [x] Вывести встроенный режим на фасад разовым входом `run_schema/3`: схема
  приходит значением, компилируется в процессе вызывающего от временного реестра
  и тут же вычисляет инстанс, а артефакт нигде не остаётся. Ни хранилища, ни
  запущенного приложения ей не нужно, зато схема обязана быть самодостаточной:
  документы хранилищ временному реестру не видны, и это закреплено тестом при
  живом стандартном хранилище. Опции обоих слоёв идут одним списком, фасад
  отделяет `output` ядру и передаёт остальное компилятору. `format_error/1`
  принял `eval_error()`, потому что из этого вызова вызывающему приходят обе
  ошибки; через него же печатается `metaschema_evaluation_failed`. Покрыто
  тестами фасада и формулировок, описано в README и okf.
- [x] Принимать схему без имени рядом, разведя регистрацию на две ступени.
  Первая, `valid_json_store:name_entries/2`, берёт только схемы и выводит имя из
  `$id`, разрешая относительный от `base_uri` хранилища; не назвавшая себя схема
  дальше не идёт и отвергается новой причиной `unnamed_schema` под ключом
  `anonymous`, потому что написания у неё нет. Вторая, `add/2`, как и прежде
  принимает только пары. Двери разведены по всей высоте — `add/1`, `store_add/2`
  и `valid_json_store_manager:add/2` берут схемы, `add_at/1,2`,
  `store_add_at/2,3` и `valid_json_store_manager:add_at/2` берут пары, — поэтому
  правило именования выбирается именем функции, а не устройством аргумента, и
  смешанных списков не остаётся нигде. Загрузчик ходит парами: путь к файлу у
  него есть. Покрыто тестами реестра, управляющего и фасада, правило имени
  описано в README и в нормативном
  `okf/architecture/validator-resources-runtime.md`, где секция про API
  размещённого режима переписана под нынешний фасад.

- [x] Собрать командный вход `valid-json`. Escript строится `rebar3 escriptize`
  из того же приложения; главный модуль назван явно в `escript_emu_args`, потому
  что по умолчанию rebar3 подставляет имя приложения, а ввод-вывод фасаду не
  принадлежит. Подкоманда `check DIRECTORY` читает каталог загрузчиком и
  проверяет набор одним проходом `valid_json_schema_set:check/2`: приложения,
  ETS и дерева супервизоров команде не нужно, метасхемы вшиты в модуль на этапе
  компиляции, поэтому escript самодостаточен и укладывается в 0,2 с холодного
  запуска. Базу имён команда строит сама из `file:`-адреса каталога: путь ей
  известен аргументом, а кодирование сегментов делает `uri_string` через
  `valid_json_uri_backend:recompose/1`; процент экранируется до этого, иначе
  `100% done` в имени каталога прочиталось бы как percent-encoding. Загрузчик
  об именах по-прежнему не знает. Продукт команды на ошибке — output units:
  схем N, проверок N и результатов N, поэтому stdout несёт поток документов
  вывода спецификации, по одному на строку и по одному на непрошедшую схему.
  Документ уходит как есть, своего в нём один член `instance` с именем
  проверенного инстанса — слота под это у вывода нет, а лишние члены его
  выходная схема не запрещает, и строка остаётся годной по ней целиком.
  Формат вывода зафиксирован на `detailed` — дерево, в котором видно, какие
  ошибки связаны между собой; выбор между `flag`, `basic` и `verbose` остаётся
  тому, кто зовёт библиотеку. Ошибка, пришедшая и своим именем, и в составе
  closure сославшегося документа, попадает в отчёт один раз. Потоки разведены
  по первичному делу команды: отчёт о валидации идёт в stdout, а всё, до
  валидации не дошедшее, — в stderr прозой `valid_json:format_error/1`.
  Разделяет их наличие output units, и другого признака у него нет. Отсюда
  коды: 0 — сказать нечего, 1 — есть схема, не прошедшая свою метасхему, 2 —
  команда не смогла что-то проверить, включая пустой каталог, потому что
  молчаливый ноль на нём был бы неотличим от проверенного набора. Рабочий вход
  `run/1` возвращает код и список выводов значением и покрыт EUnit — 16 тестов,
  и держат они только своё: разделение потоков, коды, поток документов по
  одному на схему, имя инстанса, дедупликацию closure, разбор аргументов и
  работу без запущенного приложения. Содержимое units, имена документов и слова
  `format_error/1` проверяются тестами библиотеки, и повторять их здесь
  незачем.
