...
| --- | --- | --- |
| Результат вычисления — `{валидность, аннотации, ошибки}`, а не boolean | `unevaluated*` определены через аннотации ([core.txt:828](../references/json-schema/draft-2020-12/core.txt)) | Evaluator и все обработчики |
...
| Режим short-circuit — параметр вычисления | `flag` допускает обрыв обхода, `verbose` не допускает; `contains` обязан обойти все элементы ради аннотаций | Безопасность оптимизации |
...
L3  Resource resolution       резолвер внутри ресурса — нужен рано, вместе с первыми
                              ...
                              compile/2 от терма даёт один анонимный ресурс
L4  Evaluator                 контекст: eval path, instance path, base URI,
                              dynamic scope stack, cycle guard, режим
...
| Диалект | `$schema`, допустимый только в корне ресурса ([core.txt:797](../references/json-schema/draft-2020-12/core.txt)) | набор тегов выбирается на границе ресурса |
...
`sources` — имена документов реестра, из которых собран этот терм. Не выводится из `resources`: там лежат `rid` ресурсов, включая встроенные с собственным `$id`, а нужны имена документов. Поле нужно перезаливу — по нему видно, какие артефакты устарели после изменения документа; см. «Владение и перезалив».
...
**Висячая ссылка отвергается, а не угадывается.** После сборки ресурса множество его узлов известно, поэтому `$ref` с указателем в несуществующий узел уже скомпилированного ресурса — ошибка компиляции с внятным сообщением. Междокументная ссылка проверяется там же: компиляция втягивает замыкание из `store()`, поэтому и незарегистрированный документ, и несуществующий указатель внутри него обнаруживаются на компиляции. Выбор поддержан примечанием самой спецификации: интерпретировать не-схему как схему «has security implications and may produce unpredictable results».
...
#{ root      => anonymous,
   sources   => [],
   resources => #{anonymous =>
     #resource{id = undefined, dialect = draft202012,
               anchors = #{}, dynamic_anchors = #{},
               nodes = #{<<"">> => true}}} }
```

Дальше показано только содержимое `resources` — обёртка с `root` и `sources` во всех примерах одинакова.
...
```erlang
#{ anonymous => #resource{id = undefined, dialect = draft202012,
                          anchors = #{}, dynamic_anchors = #{},
     nodes = #{
...
#{ T => #resource{id = T, dialect = draft202012, anchors = #{}, dynamic_anchors = #{},
     nodes = #{
...
`$defs` не породил ограничения — его содержимое просто попало в индекс узлов. Цикл замкнут через адреса, поэтому терм конечен, сравним и печатаем; на этом же держится cycle guard с ключом `{addr(), instance_pointer()}`.
...
```erlang
-spec new() -> store().
-spec add(store(), uri(), json()) -> {ok, store()} | {error, term()}.

-spec compile(store(), json())    -> {ok, compiled()} | {error, term()}.
-spec compile_uri(store(), uri()) -> {ok, compiled()} | {error, term()}.

-spec validate(compiled(), json(), [option()]) -> result().
```
...
Формы разделены потому, что у них разный base URI, а не ради удобства. `compile_uri/2` даёт корню каноническое имя найденного документа, и относительные ссылки внутри считаются от него. `compile/2` компилирует терм, которого в реестре нет: без `$id` он даёт `root = anonymous`, и относительная ссылка из такого корня — ошибка компиляции. Выразить одно через другое нельзя — `compile(Store, fetch(Uri, Store))` потеряет имя.

Если терм, переданный в `compile/2`, объявляет корневой `$id`, уже занятый в реестре, — это ошибка, а не переопределение. Правило то же, что в `add/3`, и по той же причине.
...
| --- | --- | --- |
| P0 | `json()`, `is_type/2` и равенство; URI и JSON Pointer; `compiled()`, контекст, output unit, формат `flag`, булевы схемы | `boolean_schema` |
...
* **Разделить эффективные аннотации и диагностическое дерево.** *(до заморозки IR)* Schema object с ложным assertion result не производит аннотаций ([core.txt:1206](../references/json-schema/draft-2020-12/core.txt)) — на этом держатся `unevaluated*`. При этом `verbose` возвращает все результаты, включая аннотации провалившихся вычислений и успехи внутри `not` ([core.txt:3254](../references/json-schema/draft-2020-12/core.txt)). Это два разных набора данных, названные в первом разделе одним словом, и тройка `{валидность, аннотации, ошибки}` их не различает. Требуется тип результата evaluator'а с раздельными полями плюс правила подъёма, объединения и отбрасывания для `allOf`, `anyOf`, `oneOf`, `not`, `if`/`then`/`else` и `$ref`.
...
* **Определить контракт dialect.** *(до P0)* `compile/2` не получает диалект. При отсутствии корневого `$schema` — а у булевой схемы его не может быть в принципе — `#resource.dialect` заполнить нечем. Спецификация оставляет этот выбор реализации, а conformance-политика задаёт исходный диалект директорией сьюта, то есть требует его передавать. Решить: параметр компиляции, поле реестра или константа плюс отдельная точка входа для раннера. Сюда же — наследует ли вложенный ресурс без `$schema` диалект лексического родителя, и что означает `optional/no-schema.json`.

* **Определить публичный контракт результата.** *(до P0)* Не заданы `result()`, `option()`, форма output unit и контекст вычисления, хотя «Опорный срез» прямо называет их дорогими к изменению. В том же узле: чем задаётся формат вывода и режим short-circuit — параметром `validate/3` или разными функциями — и как режим связан с обязательностью сбора аннотаций. Не определён и cycle guard: `instance_pointer()` в тексте есть, в типах нет, и нужно решить, входит ли в идентичность кадра dynamic scope и трактуется ли guard как множество активных кадров, а не как глобальный visited set.

* **Определить compile-time error contract.** *(до P0)* `{error, term()}` не раскрыт: несёт ли ошибка локацию, одна она или список, отличается ли ошибка регистрации от ошибки компиляции. Пока это не задано, «ошибка компиляции с внятным сообщением» приёмкой не проверяется.

* **Решить, насколько компилятор проверяет схему.** *(до P3)* Сейчас отвергаются битый `pattern` и висячая ссылка; про `maxLength: -1`, `maxLength: 2.5`, `type: "wrong"`, `$id` с фрагментом и невалидный `$anchor` не сказано ничего. Позиции: не проверять ничего сверх нужного для компиляции; проверять синтаксис каждого известного keyword'а; валидировать схему целиком против её метасхемы. Отдельными подпунктами: нераспознанный словарь со значением `true` требует отказаться обрабатывать схему ([core.txt:1423](../references/json-schema/draft-2020-12/core.txt)), и нужен явный список признаваемых URI словарей; `$vocabulary` игнорируется в документах, обрабатываемых не как метасхема; при отсутствующей метасхеме поведение остаётся за реализацией.
...
* **L1: URI и JSON Pointer.** *(до P3)* Не выбраны ни реализация, ни правила: нормализация, percent-decoding, порядок обработки fragment и JSON Pointer, пустой фрагмент, URN-базы, конфликты якорей и алиасы URI.
