...
`minLength` и `maxLength` меряют длину в characters по RFC 8259 ([validation.txt:385](../references/json-schema/draft-2020-12/validation.txt)), то есть в code points:
...
Сегмент `_/utf8` снимает один code point независимо от того, сколькими байтами тот записан и в какой кластер входит. `string:length/1` не подходит именно потому, что задаёт другой вопрос: он сегментирует строку по UAX #29 и возвращает число grapheme clusters, для чего обязан смотреть на комбинирующие классы и ZWJ. Величины совпадают не всегда:
...
Первая строка достигается парой ASCII-символов: `\r\n` есть один кластер. Нормализацией расхождение не снимается — `json:decode/1` отдаёт на `"e\u0301"` бинарник `<<101,204,129>>` как есть, и на нём `cp_length/1` даёт требуемые спецификацией 2 против 1 у `string:length/1`.

Проход тотален, и держится это на декодере: `json:decode/1` отвергает битый байт (`{invalid_byte,255}`), overlong-последовательность (`{invalid_byte,192}`) и одиночный суррогат (`"\uD83D"` → `unexpected_end`). На всём, что декодер отверг, `cp_length/1` упал бы `function_clause`; ветки на неполной последовательности нет ровно потому, что такое значение до keyword'а не доходит.

Снапшот расхождения не ловит. В `maxLength.json` и `minLength.json` обоих диалектов данные — `"\uD83D\uDCA9"`, последняя строка таблицы, где обе метрики равны единице; сценарии «two graphemes is long enough» и «one grapheme is not long enough» закрепляют, что счёт идёт не в байтах (4) и не в UTF-16 code units (2), а различие code points с кластерами не пиннят нигде. Комбинирующие последовательности во всём снапшоте встречаются только в `optional/format/idn-*`, к длине отношения не имеющих.

Перед проходом стоит оценка по `byte_size/1`. У валидного UTF-8 code point занимает от одного до четырёх байт, поэтому `byte_size(B) div 4 =< CP =< byte_size(B)`, и обе границы отвечают, не читая строку:

* `maxLength: M` — `byte_size(B) =< M` даёт true, `byte_size(B) > 4 * M` даёт false;
* `minLength: M` — `byte_size(B) < M` даёт false, `byte_size(B) >= 4 * M` даёт true.

Считать приходится только в промежутке, а там размер бинарника уже зажат keyword'ом: не больше `4 * M` байт. Отсюда два следствия. Строка в пределах `maxLength`, не содержащая ничего сверх ASCII, отвечает за O(1) — это основной случай. И работа ограничена значением keyword'а, а не размером экземпляра, причём без отдельного раннего выхода: `maxLength` не становится вектором для враждебного экземпляра, в отличие от прочего, перечисленного в пункте про ресурсные ограничения. На границах точного числа code points нет, и если сообщение об ошибке захочет назвать фактическую длину, проход делается ради него — на провалившейся ветке, не на успешной.

Отвергнут `length(unicode:characters_to_list(B))`: он строит список из N cons-ячеек, чтобы вернуть число, и платит по размеру экземпляра в обоих случаях, где `byte_size/1` не платит вовсе, — и когда ответ виден сразу, и когда строка много длиннее границы.
...
Модель значения ради этого не меняется: гибрид покрывает и обязательный набор, и оба переполненческих сценария опционального. Полным соответствием он при этом не является, и предел у него ровно один — это второй сознательный компромисс наряду с регулярными выражениями.

**Предел гибрида — десятичные дроби, не представимые двоично.** `0.3` при `multipleOf: 0.1` признаётся невалидным, тогда как спецификация определяет keyword через целочисленный результат деления ([validation.txt:341](../references/json-schema/draft-2020-12/validation.txt)). Ветвь деления даёт `2.9999999999999996`, но и ветвь точной арифметики отвечает так же: ближайший к `0.1` double — это 7205759403792794/2⁵⁶, ближайший к `0.3` — 5404319552844595/2⁵⁴, и второе не делится на первое нацело даже над bignum. Дело поэтому не в алгоритме — десятичные литералы теряет декодер, и восстановить их из double нечем. Обязательный набор этого не показывает: все одиннадцать сценариев каждого диалекта используют `2`, `1.5`, `0.0001`, `0.123456789` и `1e-8`, а `0.0075 / 0.0001` проходит потому, что округление даёт ровно `75.0`.

Точность стоит здесь замены модели значения, а не правки `multipleOf`. Сам литерал добыть недорого: `json:decode/3` принимает `#{float => Fun}`, и callback получает `<<"0.1">>`, тогда как целые проходят мимо него нетронутыми. Дорого то, что возвращённый термом литерал встаёт на место `float()` повсюду. Равенство перестаёт быть `==` — `1 == 1.0` обязано остаться истиной, — а на `==` держатся `const`, `enum`, `uniqueItems` и `dependentSchemas`. `minimum` и `maximum` перестают быть `=<` и `>=`. Печать аннотаций обязана конвертировать значение обратно, иначе наружу выйдет внутреннее представление. И `1e400` перестаёт бросать исключение, потому что с хуком приходит в callback обычным токеном, — то есть строка «Границ точности» про переполнение тоже переписывается. Половинчатый вариант не помогает: сделать точной одну схемную сторону бесполезно, экземплярный `0.3` уже равен 5404319552844595/2⁵⁴ и на точную `1/10` нацело не делится.

Отсюда заявление о соответствии: `multipleOf` вычисляется над double, и десятичная дробь, не представимая двоично, может дать ответ, расходящийся с десятичной арифметикой. Ни один тест снапшота этого не показывает, поэтому расхождение объявляется словами, а не держится красным тестом.
...
**Целочисленное значение keyword'а нормализуется на компиляции.** Восемь keywords метасхема объявляет как `nonNegativeInteger`, то есть `{"type": "integer", "minimum": 0}`, а `integer` в JSON Schema — это «any number with a zero fractional part» ([validation.txt:297](../references/json-schema/draft-2020-12/validation.txt)). Поэтому `"maxLength": 2.0` — законное значение, и в основном наборе каждого диалекта оно стоит у всех восьми: `maxLength`, `minLength`, `maxItems`, `minItems`, `maxProperties`, `minProperties`, `minContains`, `maxContains`.

Приёмка такого расхождения не обнаружит: `=<` и `>=` сравнивают целое с дробным, поэтому все шестнадцать файлов прошли бы и без всякой нормализации. Она нужна по трём другим причинам. Первая — объявленный слот: тип `non_neg_integer()` обязывает хранить целое, иначе он описывает не то, что в терме на самом деле лежит. Вторая — тождество термов: `{max_length, 2}` и `{max_length, 2.0}` равны по `==`, но различны по `=:=`, так что две одинаковые по смыслу схемы дали бы разные артефакты, а на сличении терма с эталоном держится вся приёмка. Третья — дальнейшее употребление границы: сегодня её только сравнивают, но `lists:split(2.0, L)` завершается `badarg`, и ничто не обещает, что сравнением дело и ограничится.

Вопрос здесь тот же, что в «Различении целого и дробного», только заданный схеме, а не экземпляру:

```erlang
norm_int(V) when is_number(V), V == trunc(V), V >= 0 -> trunc(V).
```

Первой в guard'е стоит проверка `is_number/1`, и она не формальность: без неё `"maxLength": "2"` вызовет `badarg` внутри `trunc/1`, то есть падение компилятора вместо отказа компиляции. С нею всё непредставимое — строка, `null`, `2.5`, `-1` — одинаково не совпадает с единственным clause. Это та же функция, что `is_type(integer, V)`, только записанная короче: `is_integer(V) orelse (is_float(V) andalso V == trunc(V))` эквивалентно `is_number(V) andalso V == trunc(V)`. На вычислении дробного значения уже не существует.

Правило распространяется только на эти восемь keywords. `minimum`, `maximum`, `exclusiveMinimum` и `exclusiveMaximum` метасхема объявляет как `{"type": "number"}`, целочисленности они не требуют, и `2.0` для них — обычное значение: оно ложится в ограничение таким, каким пришло от декодера, а сравнение разбирается с целым и дробным само.

Значение, которое не нормализуется, отвергается компиляцией. Это не выбор строгости, а следствие уже принятых решений. Расширить слот до `number()` значило бы хранить то, что запрещает метасхема, ради сравнения, которое всё равно даёт тот же ответ. Проигнорировать keyword не позволяет правило «написанный keyword всегда даёт ограничение». Остаётся отказ, и его причина — `{bad_keyword_value, json()}`. Имя выбрано общим намеренно: будущее решение о том, сколько ещё синтаксиса проверяет компилятор, добавит к этой причине новых производителей, но не потребует заводить новую. Само значение хранится внутри причины по тому же доводу, что и у `bad_pattern`: текст сообщения вычисляется из записи, а исходной схемы в скомпилированном терме уже не остаётся.

Та же причина отвергает `multipleOf` со значением, не большим нуля, — метасхема объявляет его как `{"type": "number", "exclusiveMinimum": 0}`. Здесь довод даже сильнее, чем для `2.5`: `V rem 0` и `V / 0` завершаются `badarith`, то есть пропущенное значение не портит ответ, а обрывает вычисление, причём обе ветви гибридного алгоритма падают одинаково. Снапшот и этого не покажет — в нём встречаются только `2`, `3`, `5`, `1.5`, `0.5`, `0.0001`, `0.123456789` и `1e-8`.
...
-type reason() ::
    ...
    | {bad_keyword_value,       json()}     % значение keyword'а непредставимо в слоте
