...
- Владение таблицами. Таблиц у хранилища две — артефакты и реестр, — и на каждую приходится свой хранитель, держащий её между воплощениями управляющего. Управляющий забирает обе через `ets:give_away/3` и единственный в них пишет. Storage сведён к `lookup`, `put` и `delete` над переданными таблицами. Разбор — в [ETS и процессах](validator-resources-runtime.md#ets-и-процессы).
