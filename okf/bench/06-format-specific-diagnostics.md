...
После удаления diagnostic tree Basic больше не является главным источником
собственной служебной структуры. Следующий локальный кандидат виден в
`valid_json_assert:report/4`: в Basic успешный assertion не имеет ни error, ни
annotation, но сейчас всё равно проходит через `detail/3`, `keyword/1`,
`keyword_units/5` и создаёт новый `#eval_result{}`. На этом сценарии таких
вызовов почти семьсот, а один `report/4` выделяет ровно шесть words.

Безопасный следующий эксперимент — fast path
`report(_, true, _, basic) -> empty_result(true)`. Верхняя оценка прямого
выигрыша только по `report/4` — около 4 200 words, или 33.6 KB/op, плюс
сокращение вызовов `detail/3`, `keyword/1` и `keyword_units/5`. После него снова
нужно профилировать `eval_at/6` и object traversal: это уже общие расходы
evaluator, одинаково важные для Basic и Flag.
