...
Секция названа для каждого attribute, который валидатор проверяет сам. Имена с отложенным решением и имена вне IETF перечислены ниже отдельно.
...
RFC 3987 (IRI), RFC 5890 и RFC 5891 (IDNA), RFC 6531 (SMTPUTF8) нормативны для четырёх format attributes — `iri`, `iri-reference`, `idn-hostname` и `idn-email`, — решение по которым отложено. Пока эти имена остаются чистой annotation, тексты валидатору не нужны; они появятся здесь вместе с решением их реализовать.
