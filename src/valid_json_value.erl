...
%% uniqueItems держится на той же JSON equality, что const и enum. Канонизация
%% чисел для этого не нужна: `lists:usort/1` отсеивает элементы, равные по
%% термовому порядку, а он сравнивает числа по значению и потому считает 1 и 1.0
%% одним элементом — в том числе вложенными в массивы и объекты. Свойства, на
%% которых держится замена, закреплены тестами.
%%
%% Сравнений теперь порядка n log n вместо n²: на пяти тысячах элементов это
%% доли миллисекунды против сотен. Взамен теряется обрыв на первом же
%% совпадении — массив с дублем в начале раньше отвергался мгновенно, а теперь
%% сортируется целиком. Проигрыш в десятки микросекунд и только на массиве,
%% который и так признан негодным.
-spec is_unique([json()]) -> boolean().
is_unique(Items) ->
    length(lists:usort(Items)) =:= length(Items).
